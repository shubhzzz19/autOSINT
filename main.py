...
class Main:
       def __init__(self,win):
              ...
              self.label1 = Label(self.root,text='A cutting-edge Footprinting software that leverages AI and Machine Learning techniques to extract valuable information about target victims using OSINT',font=fnt6,fg='#443565')
              self.label1.place(x= 240,y=95)
              self.label1 = Label(self.root,text='(Open Source Intelligence) APIs. The software provides a user-friendly GUI developed with Python Tkinter, enabling users to interact and input their requirements to obtain essential',font=fnt6,fg='#443565')
              self.label1.place(x= 100,y=120)
              self.label1 = Label(self.root,text='details about the target victims. These details include location information, phone numbers, domain information, and more. AutOSINT offers a powerful and efficient solution for',font=fnt6,fg='#443565')
              self.label1.place(x= 100,y=145)
              self.label1 = Label(self.root,text='gathering crucial intelligence, aiding in investigations and data analysis.',font=fnt6,fg='#443565')
              ...
              self.nm_org = Label(self.root,text='Name of Organization:',font=fnt2,fg='#443565')
              ...
              self.fetch = Button(self.root,text='Fetch',font=fnt4,bg='white',command=self.fetch)
              self.fetch.place(x=450, y=555)
              
              self.exit = Button(self.root,text='Exit',font=fnt4,bg='white',command=self.exit_app)
       ...
       def fetch(self):
              #Progress Bar
              self.progbar = Progressbar(self.root, orient=HORIZONTAL, length=220, mode="determinate")
              self.progbar.place(x=490,y=487)
              self.progbar.start()
